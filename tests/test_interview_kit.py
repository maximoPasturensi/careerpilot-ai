...
def test_interview_kit():
    ...
    print("4/4 - Generando kit de entrevista...\n")
    kit = generate_interview_kit(cv, job, match)

    for i, qa in enumerate(kit.questions, 1):
        print(f"--- Pregunta {i} [{qa.question_type.upper()}] ---")
        print(f"P: {qa.question}")
        print(f"R: {qa.suggested_answer}")
        print(f"Tip: {qa.tip}\n")

    print(f"=== NOTA DE PREPARACION ===\n{kit.preparation_note}")
